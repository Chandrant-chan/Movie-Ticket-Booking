...
def movies_list(city):
    ...
    btn1=Button(city,text='THE BOYS \n SEASON 5', **button_style1(),command=lambda: seat_selection(city,"THE BOYS SEASON 5"))
    btn1.pack(side=LEFT, padx=15, pady=10, anchor=CENTER)

    btn2=Button(city,text='SQUID GAME 2', **button_style1(),command=lambda: seat_selection(city,"SQUID GAME 2"))
    btn2.pack(side=LEFT, padx=15, pady=10, anchor=CENTER)
    
    btn3=Button(city,text='KGF 2', **button_style1(),command=lambda: seat_selection(city,"KGF 2"))
...
def print_ans(lbl_text,menubutton,label,city):
    ...
    menubutton.config(textvariable=changecity, font=('Arial', 9, 'bold'), width=8, height=1, bd=4,
                  bg='pink', fg='black', activebackground='yellow', activeforeground='WHITE')
...
def main_account_screen():
    ...
    main_screen.title('TICKET BOOKING SYSTEM')
    Label(main_screen,text="TICKET BOOKING SYSTEM",bg="yellow",fg="black",height='2',
          font=label_font,width=label_width).pack()
    Label(main_screen,text='',bg="white").pack()
